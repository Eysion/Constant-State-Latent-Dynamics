...
每个 Run 都必须先完成 context-review，再进入 planning；OpenSpec 只在契约信号命中时强制。context-review 使用 `tooling/ai-workflow/templates/planning/context-review-evidence.json`，必须记录已阅读的仓库指导路径、至少一个带行号和用途的源码引用，以及所选子仓库实际使用的 test/lint/build/check 验证命令：
...
通过 `--planning-mode` 可以显式升级，契约类信号不得降级。planning 必须包含实施步骤和可执行验证项；implementation 必须完整覆盖非生成 owned paths 并说明可执行验证范围。计划由模型生成和更新，OpenSpec 保存长期契约，父级状态机保存执行和证据，三者不是替代关系。
