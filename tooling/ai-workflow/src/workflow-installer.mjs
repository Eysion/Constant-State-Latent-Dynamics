...
function renderSingleConfig(detection) {
  ...
  return [
    ...
    'verification:',
    `  file: ${JSON.stringify(command.file)}`,
...
function renderEvidenceRoutingPolicy(detection, indentation = 0) {
  ...
  const verificationCost = detection.verification.file === 'git' ? 0 : 1
  return [
    ...
    `${prefix}      timeoutMs: ${detection.verification.timeoutMs}`,
  ]
}

function renderQualityPolicy(detection, indentation = 0) {
  const prefix = ' '.repeat(indentation)
  const candidateCount = detection.verification.file === 'git' ? 2 : 3
  const accuracyThreshold = detection.verification.file === 'git' ? 0.9 : 0.85
  return [
    ...
    `${prefix}  accuracyThreshold: ${accuracyThreshold}`,
  ]
}
...
function renderVerification(command, indentation) {
  ...
  return [
    `${prefix}verification:`,
    `${prefix}  file: ${JSON.stringify(command.file)}`,
    `${prefix}  args: [${command.args.map((value) => JSON.stringify(value)).join(', ')}]`,
    `${prefix}  timeoutMs: ${command.timeoutMs}`,
  ]
}
...
function shouldCopy(sourceDirectory, sourcePath) {
  ...
  return !['node_modules', 'state', 'coverage'].includes(first)
}
...
async function installDependencies(engineDirectory, warnings) {
  ...
  warnings.push('pnpm was not available; npm installed from package.json without changing the distributed pnpm lock')
