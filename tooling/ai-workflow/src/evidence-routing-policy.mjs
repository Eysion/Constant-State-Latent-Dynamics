...
export function resolveEvidenceRoute({ policy = DEFAULT_EVIDENCE_ROUTING_POLICY, qualityGate } = {}) {
  ...
  return {
    ...
    capabilities: policy.capabilities.map((item) => ({ id: item.id, kind: item.kind, access: item.access, enabled: item.enabled, costUnits: item.costUnits, timeoutMs: item.timeoutMs })),
    modelCandidates: qualityGate?.enabled ? qualityGate.candidateCount : 1,
...
export function summarizeToolBudget(record, route) {
  const completed = record.events.filter((event) => event.type === 'TOOL_CALL_FINISHED')
  const mcpCalls = completed.filter((event) => event.evidence.kind === 'mcp').length
  const costUnits = completed.reduce((total, event) => total + event.evidence.costUnits, 0)
  return {
    mcpCalls,
    costUnits,
