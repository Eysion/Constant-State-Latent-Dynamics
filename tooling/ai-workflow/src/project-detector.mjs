...
function nodeVerification(manager, scripts = {}) {
  const script = ['test', 'build', 'lint'].find((candidate) => {
  ...
  if (manager === 'pnpm' || manager === 'yarn') {
    return { file: manager, args: [script], timeoutMs: 120_000 }
  }
  return { file: manager, args: ['run', script], timeoutMs: 120_000 }
}
...
export async function detectProject(directory) {
  ...
  return {
    ...
    manifests,
    verification,
    verificationCommand: displayCommand(verification),
    warnings,
  }
}
