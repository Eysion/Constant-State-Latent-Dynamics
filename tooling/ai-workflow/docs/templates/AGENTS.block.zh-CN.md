...
- `structured`：多步骤、多组件、多验收项、可恢复任务或长任务；以 `tooling/ai-workflow/templates/planning/structured-plan.json` 为模板写入 `.workflow/plan.local.json`。
...
node tooling/ai-workflow/cli/workflow.mjs resolve \
  --node planning \
  --evidence-file .workflow/plan.local.json
```
...
Loop 在 `loop-execution` 节点内执行 `implement → check → inspect → adjust`，默认最多 6 轮、连续 2 轮无进展即停止、总时间 60 分钟。验收通过后用 `tooling/ai-workflow/templates/planning/loop-checkpoint.json` 写入迭代及最终证据；状态机拒绝缺少证据、超轮次、超时或未达到验收的成功结果。预算耗尽、无进展或需要人为判断时应停止并等待用户。

### Quality gate

安装器会按已识别的项目验证能力生成 `qualityPolicy`。当任务携带配置中的高风险或负反馈信号时，工作流自动加入 `quality-assessment`：比较多个独立候选，并以检索、源码复核、验证或独立审阅证据确认准确性阈值。用户不需要选择模型参数；低于阈值不得标记为成功，必须继续验证或澄清。业务风险信号仍须由项目规则或任务事实声明，安装器不会猜测业务领域。
