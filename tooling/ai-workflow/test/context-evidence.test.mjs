...
test('validates repository context review and actionable implementation evidence', () => {
  assert.equal(validateNodeEvidence({ nodeId: 'context-review', context, evidence: {
    schemaVersion: 1, kind: 'context-review', guidancePaths: ['src/feature.mjs'],
    sourceReferences: [{ path: 'src/feature.mjs', line: 1, purpose: 'Review implementation' }], verification: ['node test'],
  }}).ok, true)
  assert.equal(validateNodeEvidence({ nodeId: 'context-review', context, evidence: {
    schemaVersion: 1, kind: 'context-review', guidancePaths: ['src/feature.mjs'],
    sourceReferences: [{ path: 'src/feature.mjs', line: 1, purpose: 'Review implementation' }], verification: ['manual inspection only'],
  }}).ok, false)
})
...
test('requires evidence-backed quality decisions to honor the Run threshold', () => {
