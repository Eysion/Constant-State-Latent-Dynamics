...
test('detects ignored files before workflow commit planning', async (t) => {
  const directory = await temporaryRepository(t)
  await writeFile(path.join(directory, '.gitignore'), 'ignored.txt\n')
  await writeFile(path.join(directory, 'ignored.txt'), 'ignored\n')
  const { git } = setupAdapter(directory)

  assert.equal(await git.isIgnored('ignored.txt'), true)
  assert.equal(await git.isIgnored('first.txt'), false)
})
