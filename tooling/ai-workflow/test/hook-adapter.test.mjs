...
test('denies file mutation before an active workflow run exists', () => {
  ...
  assert.equal(evaluateHook('pre-tool-use', { tool_name: 'Edit', tool_input: { file_path: '/repo/owned.js' } }, { activeRun, repositoryDirectory: '/repo' }).decision, 'allow')
  assert.equal(evaluateHook('pre-tool-use', { tool_name: 'Edit', tool_input: { file_path: 'owned.js' } }, { activeRun, repositoryDirectory: '/repo' }).decision, 'allow')
  assert.equal(evaluateHook('pre-tool-use', { tool_name: 'Edit', tool_input: { file_path: './owned.js' } }, { activeRun, repositoryDirectory: '/repo' }).decision, 'allow')
  assert.equal(evaluateHook('pre-tool-use', { tool_name: 'Edit', tool_input: { file_path: '../other.js' } }, { activeRun, repositoryDirectory: '/repo' }).decision, 'deny')
  assert.equal(evaluateHook('pre-tool-use', { tool_name: 'Edit', tool_input: { file_path: '/repo/other.js' } }, { activeRun, repositoryDirectory: '/repo' }).decision, 'deny')
